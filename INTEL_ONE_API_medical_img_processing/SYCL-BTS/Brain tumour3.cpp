...
// Kernel function to detect brain tumors
void brainTumorDetection(const float* input, int* output, const float threshold, const int dataSize) {
...
int main() {
    ...
    deviceQueue.submit([&](handler& cgh) {
        ...
        // Kernel function
        cgh.parallel_for<class BrainTumorDetectionKernel>(range<1>(dataSize), [=](id<1> idx) {
